...
import com.company.service.PeopleMemberService;
...
public class PeopleMemberController {
	...
	public String getModify(@PathVariable("sitename") String sitename, HttpServletRequest req, Model model)
		...
		PeopleMemberDTO mo = service.getModify(sitename, m.getUserno());

		model.addAttribute("sitename", sitename);
		model.addAttribute("mo", mo);
		return "people/member/modify";
	}

	// 회원정보 수정 post
	@RequestMapping(value = "/{sitename}/modify", method = RequestMethod.POST)
	public String postModify(@PathVariable("sitename") String sitename, PeopleMemberDTO dto) throws Exception {
