...
<body>
	...
	<c:forEach items="${dtos}" var="dtos">
		<c:choose>
			...
			<c:when
				test="${dtos.getStatus() eq 'deploy' && dtos.getCategory() eq 'allba'}">
				...
				<div class="card">
                	...
                	<div style= "text-align : center">
                		<button class='btn btn-success' style = "width : 48%" style = "float : left" onclick = "location.href ='/allba/${dtos.getSitename()}'">접속</button>
                		<button class='btn btn-danger' style = "width : 48%" style = "float : right" onclick = "location.href ='/dashboard/deletesite?siteid=${dtos.getSiteid()}&status=${dtos.getStatus()}&sitename=${dtos.getSitename()}'">삭제</button>
                	</div>
            	</div>
			</c:when>
